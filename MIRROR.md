...
| Elementor Pro enquiry forms — **31 instances, 10 distinct, on 30 pages** (contact, book a call, expo signup, demo, partner) | was `admin-ajax.php`; now `/_forms/submit.php` and `/_forms/forms.js`, **which need a host that runs PHP and the private config** | **Wired, answering in place as Elementor did.** See the note below the table |
...
**Since 22 September the forms behave as they did on WordPress, and their mail arrives.**
The site went live on the 21st with the handler above calling PHP `mail()`, and enquiries
did not arrive: Hostinger's mail is unsigned for waterautomation.com, whose SPF ends `-all`
and whose DMARC is `p=quarantine`, so Microsoft 365 quarantined it. And every visitor was
sent to `/thank-you/`, where Elementor had answered under the form. Now:

- `/_forms/forms.js` (generated by `npm run forms:wire`, loaded on the 30 form pages) does
  what Elementor's handler did, in Elementor's markup and words: the post goes by `fetch`,
  the form fades and the button shows `elementor-form-spinner`, and "Your submission was
  successful." appears under the form as `elementor-message elementor-message-success`
  before the fields clear. Refusals mark the field with `elementor-error`. Without
  JavaScript the form still posts and lands on `/thank-you/`.
- The handler writes every validated enquiry to `private/submissions-YYYY-MM.jsonl`
  **before** any email is tried, then sends it through `_lib/wa.php`: Resend first, then
  Microsoft Graph and SMTP if configured, then `mail()` as a last resort. The outcome is
  appended under the same id, which is also the `Ref:` line in the email.
- The /case-studies/ form, the one of ten with no spam trap, has one, and every phone
  field has a pattern browsers actually apply (the Elementor one is a syntax error under
  the RegExp `v` flag current browsers use, so it was ignored).

**What production needs**, all outside the repo:

1. `private/config.php` beside `public_html` (the path is
   `dirname($_SERVER['DOCUMENT_ROOT'])/private/config.php`), mode 0600, never committed.
   `tools/config.example.php` documents every key: `mail.resend.api_key`, `mail.from`,
   `forms.to` and, if the WordPress forms emailed different people, `forms.to_by_form`.
   With no file, the forms still work exactly as on the 21st, `mail()` and all.
2. waterautomation.com verified in Resend: the records Resend lists for it (a DKIM TXT at
   `resend._domainkey`, and an MX and SPF TXT on the `send.` subdomain) added in
   Cloudflare as DNS only. They sit beside the Microsoft 365 records and change none.
3. One test submission after deploy, then a look at the log. The folder and its files are
   created on first use (0700 and 0600); `grep '"ok":false' private/submissions-*.jsonl`
   lists any enquiry whose email did not go out, and the commands at the foot of
   `tools/config.example.php` print the log as a table. Spam-trap hits are in
   `private/spam-YYYY-MM.jsonl`.
